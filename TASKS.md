...
| U1.0 | done | Backfill chunks for pre-vector-layer sessions (+ fix the Gemini 100-per-batch embed limit) | **Row reconciled from `.goal/goal.json` by the `lane/c-unrun-writers` session, not authored by it** — this is the other maker loop's unit and `tracker-audit --gate G1` was failing on the row-set mismatch. Its own evidence: `qa/verdicts/chunk-backfill.md` PASS 8/8 cycle 1 (`226fa06`); live chunks 0 to 1452, 26/26 sessions, 0 dim mismatches, 2118/2118 turnRefs resolve; discharges C8 and ISS-113, fixes ISS-116. |
