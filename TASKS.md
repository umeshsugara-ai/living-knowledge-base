...
| U2.1c | in_progress | Tenant-namespace entity ids (entity-promotion contract C6) | SECOND instance of the ISS-121 shape, in a file I wrote one unit after fixing the first. `scopedCollection` merges tenantId into the FILTER but Mongo's `_id_` index is unique per COLLECTION, so tenant B upserting a slug tenant A holds gets E11000 - reproduced live before fixing. Worse than ISS-121: it sits inside the never-throws catch, so tenant B would lose ALL entity promotion silently and permanently. |
