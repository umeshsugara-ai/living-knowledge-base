...
- **[I7] Staleness is caught.** `--check` must exit non-zero when the committed doc differs from a
  fresh regeneration, and must be wired into the commit/CI path — `pnpm lint:score`, invoked as its
  own step in `.github/workflows/ci.yml` — not into `pnpm lint:structure`, which stays runnable at
  any moment during an in-flight unit (ISS-058).
...
| 2026-09-07 | routine (reconcile wording, no substance change) | **Rewrote I7's own text** — "must be wired into `pnpm lint:structure`" → "must be wired into the commit/CI path — `pnpm lint:score`, invoked as its own step in `.github/workflows/ci.yml` — not into `pnpm lint:structure`" | Landed as part of the `lint-score-split` unit (Mode A check, cycle 1): the unit did exactly what the prior amendment-log ruling authorized (split `catalogue-score --check` out of `lint:structure` into `lint:score`, gated on CI), and I7's literal sentence still named the OLD wiring, so a reader of I7 alone would have read a compliant unit as a violation. The ruling already decided the substance; this entry only makes I7's own text match the ruling it was already bound by. No criterion loosened: the refusal-on-uncommitted-input strength (I11–I14) is untouched, and I7 is, if anything, more precise now (names the exact script and step) than before. |
